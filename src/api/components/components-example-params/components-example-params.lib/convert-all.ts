...
export function convertDataBdToReturn(data, type = TypeConvertInterface.array, validate?:ComponentsContent[]) {
...
function convertDataBdArray(data, validate:ComponentsContent[]) {
 const convert_validate = convertValidate(validate);
  ...
  data.forEach((e) => {
    ...
    res[index][e.name_params] = validateConvertType(e.value, convert_validate[e.name_params])
  });
  return res;
}
...
export function convertParamsContentUpdate(
  ...
  validate: ComponentsContent[]
    ...
    elem.filter((e: ComponentsContentExampleParams) => e.id === id)[0].value = data.value;
